...
typedef struct
{
	...
	BYTE szRequestKeyDate[DATE_BCD_SIZE+1];     /* BCD MMDD        */
	BYTE szUserName[12+1];
	BYTE szAcquirerFee[12+1];

    BOOL fAtmMsg;
	/*POS*/
	BYTE szPOSCashInAcquirerFeeOnUs[12+1];
	BYTE szPOSCashOutAcquirerFeeOnUs[12+1];
	BYTE szPOSCashOutAcquirerFeeOffUs[12+1];

    /*ATM*/
	BYTE szATMCashOutAcquirerFeeOnUs[12+1];
	BYTE szATMCashOutAcquirerFeeOffUs[12+1];
	
	BYTE szBalInqAcquirerFee[12+1];
	BYTE szBillsPaymentServiceFee[12+1];
...
{
	int	BLRid;
	BYTE	szBillerName[36];	/*  */
	BYTE	szBillerCode[4];	/* ASCII string */		
	BYTE	szBillerFee[12];
} STRUCT_BLR;
