	...
	if(d_OK == usResult)
	{
        ...
        if (memcmp(bOutBuf,SHARE_EMV_SUB_IPC,3) == 0)
    	{
    		if (memcmp(&bOutBuf[3],SHARE_EMV_ONLINEPIN,1) == 0)
    		{
				...
				if(d_OK != usResult)//Do not return. Need to perform inMultiAP_RunIPCCmdTypes again even if user cancels PIN entry screen.
					usResult2 = usResult;
					//return usResult;
				...
				if(d_OK != usResult2)//Need to perform inMultiAP_RunIPCCmdTypes even if user cancels PIN entry screen.
                    return usResult2;
